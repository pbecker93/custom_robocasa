...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def reset_to(self, state):
        ...
        if "model" in state:
            ...
            if hasattr(self, "set_attrs_from_ep_meta"):  # older versions had this function
                self.set_attrs_from_ep_meta(ep_meta)
            elif hasattr(self, "set_ep_meta"):  # newer versions
                self.set_ep_meta(ep_meta)
            ...
            if not self._is_v1:
                ...
                self.sim.model.site_rgba[self.eef_site_id] = np.array([0.0, 0.0, 0.0, 0.0])
                self.sim.model.site_rgba[self.eef_cylinder_id] = np.array([0.0, 0.0, 0.0, 0.0])
        if "states" in state:
